...
void loop() {  
  ...
  if(SerialUSB.available()) {
    if(SerialUSB.read()=='s') {
        ...
        { 
          while (fsrADC != 0) // If the analog reading is non-zero
          {
            ...
            if (fsrR <= 600) 
              ...
              if(force > 50){
                ...
                AX12_state[1] = Wire.read();
                AX12_state[0] = Wire.read();
                x = AX12_state[1];
                y = AX12_state[0];
                fsrADC=0;
              }
    ...
    else
    {
     ...
     if(SerialUSB.available() ) {
        ...
        atime_final[3]  = SerialUSB.read() - 48;
      } 
      time_final = (atime_final[0] * 10 + atime_final[1] + atime_final[2]/10 + atime_final[3]/100);
